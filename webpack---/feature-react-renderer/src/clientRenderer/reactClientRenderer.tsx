...
export const ReactClientRenderer = withDependencies(
	...
	(
		...
		render: async (target = document.getElementById('SITE_CONTAINER') as HTMLElement, rootCompId = 'main_MF') => {
			...
			const App = require('../components/App').default // App creates a React Context on mo`dule state, so it has to be evaluated once React is defined.
			let ComponentsCss
			if (process.env.PACKAGE_NAME === 'thunderbolt-ds' || process.env.NODE_ENV === 'test') {
				ComponentsCss = require('../components/ComponentsCss').ComponentsCss // ComponentsCss creates a React Context on module state, so it has to be evaluated once React is defined.
			} else {
				ComponentsCss = _.noop
			}
