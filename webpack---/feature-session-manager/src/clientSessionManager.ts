...
export const SessionManager = withDependencies(
	...
	(
		...
		const sessionModel: Partial<DynamicSessionModel> = Object.assign(siteFeatureConfig.sessionModel, dynamicModel)
		...
		const loadNewSession: ISessionManager['loadNewSession'] = async (
			{ reason, authorizationCode } = { reason: 'noSpecificReason' }
		) => {
			try {
